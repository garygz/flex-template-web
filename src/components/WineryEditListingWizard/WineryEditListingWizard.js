...
import WineryEditListingWizardTab, {
    TAB1_WINE_DETAILS,
    TAB2_DISTRIBUTION_INFO,
    TAB3_ADDITIONAL_ATTRIBUTES,
    PHOTOS
} from './WineryEditListingWizardTab';
...
const tabLabel = (intl, tab) => {
  ...
  // }
  if (tab == TAB1_WINE_DETAILS) {
    key = 'WineryEditListingWizard.tab1_wine_details';
  } else if (tab == TAB2_DISTRIBUTION_INFO) {
    key = 'WineryEditListingWizard.tab2_distr_info';
  } else if (tab == TAB3_ADDITIONAL_ATTRIBUTES) {
    key = 'WineryEditListingWizard.tab3_add_attributes';
  } else if (tab == PHOTOS) {
    key = 'WineryEditListingWizard.photos';
  }
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    case TAB1_WINE_DETAILS:
      return !!(productName && title && vintage && wineType);
    case TAB2_DISTRIBUTION_INFO:
      return !!(publicData && publicData.distribution);
    case TAB3_ADDITIONAL_ATTRIBUTES:
      return !!(publicData && publicData.additionalAttributes);
    // case POLICY:
...
class WineryEditListingWizard extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    if (!tabsStatus[selectedTab]) {
      ...
      return <NamedRedirect name="WineryEditListingPage" params={{ ...params, tab: nearestActiveTab }} />;
    }
    ...
    const tabLink = tab => {
      return { name: 'WineryEditListingPage', params: { ...params, tab } };
    };
...
WineryEditListingWizard.defaultProps = {
  className: null,
  rootClassName: null,
  listing: null,
};

WineryEditListingWizard.propTypes = {
